...
def current_weather(request):
    if request.method == 'POST': 
        ...
        if cached_data:
            ...
            source = urllib.request.urlopen('https://api.openweathermap.org/data/2.5/weather?lat='+ latitude + '&lon=' + longitude + '&appid=352350089cd0815f99eb9fc26635d7fd').read()
    ...
    if request.method == 'POST': 
        ...
        if cached_data:
            ...
            source = urllib.request.urlopen('https://api.openweathermap.org/data/2.5/forecast/daily?lat='+ latitude + 
                                 '&lon=' + longitude +'&type=hour&appid=ca5a8574962b6e08edf9c49d364192fa').read()
            list_of_data = json.loads(source)
